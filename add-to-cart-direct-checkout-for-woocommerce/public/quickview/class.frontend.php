...
class pisol_dcw_quick_view_frontend{
	
	public $pisol_plugin_dir_url;
    public $pisol_options;
    public $pisol_style;
	public $pi_dcw_quick_view_text;
	public $pi_dcw_quick_view_modal_bg_color;
	public $pi_dcw_quick_view_modal_padding;
	public $pi_dcw_quick_view_modal_text_color;
	public $pi_dcw_quick_view_modal_close_bg_color;
	public $pi_dcw_quick_view_modal_close_color;
	public $pi_dcw_quick_view_bg_color;
	public $pi_dcw_quick_view_text_color;
	...
	function __construct($pisol_plugin_dir_url){
		...
		$this->pi_dcw_quick_view_modal_close_color = get_option('pi_dcw_quick_view_modal_close_color', '#ffffff');
  		...
  		$this->pi_dcw_quick_view_bg_color = esc_html(get_option('pi_dcw_quick_view_bg_color', '#ee6443'));
  		$this->pi_dcw_quick_view_text_color = esc_html(get_option('pi_dcw_quick_view_text_color', '#ffffff'));
        ...
        add_action('pisol_show_product_sale_flash','woocommerce_show_product_sale_flash',10);
        add_action('pisol_show_product_images', 'woocommerce_show_product_images',20);
	...
	public function pisol_add_button(){
		...
		global $post;
        echo '<a data-product-id="'.esc_attr( $post->ID ).'"class="quick_view button pisol_quick_view_button" >
        <span>'.esc_html($this->pi_dcw_quick_view_text).'</span></a>';
	}
	...
	public function pisol_get_product(){
		...
		if(intval($product_id)){
	 	    ...
	 	    <script>
		 	    var url = "<?php echo esc_url( $this->pisol_plugin_dir_url ); ?>/js/prettyPhoto.init.js";
		 	    var wc_add_to_cart_variation_params = <?php echo json_encode($parameter); ?>;     
			...
			if(false): 
			?>
 	        <div class ="pisol_prev_data" data-pisol-prev-id = "<?php echo esc_attr( $prev_post_id ); ?>">
 	        <?php echo wp_kses_post( $prev_post_title ); ?>
 	            <?php echo wp_kses_post( $prev_thumbnail ); ?> 
 	        </div> 
 	        <div class ="pisol_next_data" data-pisol-next-id = "<?php echo esc_attr( $next_post_id ); ?>">
 	        <?php echo wp_kses_post( $next_post_title ); ?>
 	             <?php echo wp_kses_post( $next_thumbnail ); ?> 
			 </div> 
			<?php endif; ?>
		</div>
 	        <?php
	...
	public function avoid_redirect_to_single_page( $value ) {
		if ( isset($_REQUEST['action']) && $_REQUEST['action'] == 'pisol_get_product') {
			return '';
		}
		return $value;
	}
...
if($pi_dcw_enable_quick_view_button == 1){
	new pisol_dcw_quick_view_frontend(plugin_dir_url( __FILE__ ));
}
?>